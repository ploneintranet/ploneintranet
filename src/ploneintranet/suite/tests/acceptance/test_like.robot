...
I toggle like on the first status update
    Click element  xpath=//div[@class='post status item'][1]//button

The first status update is not liked
    Element should be visible  xpath=//div[@class='post status item'][1]//sup[@class='counter' and contains(text(), '(0)')]

The first status update is liked once
    Element should be visible  xpath=//div[@class='post status item'][1]//sup[@class='counter' and contains(text(), '(1)')]

The first status update is liked 2 times
    Element should be visible  xpath=//div[@class='post status item'][1]//sup[@class='counter' and contains(text(), '(2)')]

The first status update is liked 3 times
    Element should be visible  xpath=//div[@class='post status item'][1]//sup[@class='counter' and contains(text(), '(3)')]
