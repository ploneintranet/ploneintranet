...
from .. import testing
from .base import SiteSearchTestBaseMixin
...
class IntegrationTestMixin(SiteSearchTestBaseMixin):
...
SiteSearchTestBaseMixin.register(IntegrationTestMixin)
...
class TestSiteSearchPermissions(IntegrationTestMixin,
                                SiteSearchTestsMixin,
                                testing.IntegrationTestCase):
    """Integration tests for site search permissions.
    ...
    def setUp(self):
        super(TestSiteSearchPermissions, self).setUp()
        pa_testing.login(self.layer['portal'], pa_testing.TEST_USER_NAME)

    def test_documents_owned_by_other_not_visible_same_role(self):
        with testing.login_session(testing.TEST_USER_1_NAME):
            util = self._make_utility()
    ...
    def test_group_changes(self):
        ...
        api.group.add_user(
            groupname='TestUsers',
            username=testing.TEST_USER_1_NAME)
